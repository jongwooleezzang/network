...
class ManagerLogin extends JFrame implements ActionListener, KeyListener {	//GUI 관리자 창 
	...
	public ManagerLogin() {
		...
		port_label.setForeground(Color.RED); // 빨간색
        port_warn.setForeground(Color.blue); // 파란색
		port_label.setFont(font);
		port_warn.setFont(font);
		setTitle("GUI 관리자 창");
...
class ChatServerGUI extends JFrame implements ActionListener, KeyListener {
	...
	TextArea serverChatList = new TextArea(30, 30);
	TextArea userList = new TextArea(30, 30);
	...
	public void keyPressed(KeyEvent e) {
		...
		if(e.getKeyCode() == KeyEvent.VK_ENTER && message.length() > 0) {
			appendMessage("전지전능한관리자 : "+message+"\n");
			sb.transmitAll("전지전능한관리자 : "+message+"\n");
			chat.setText(null);
		}
	}
